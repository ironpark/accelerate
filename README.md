...
Every code sample below lives in [`examples/readme.zig`](examples/readme.zig)
and is compiled by `zig build test`, so a stale snippet turns the build red
...
| | |
|---|---|
| **Errors** | Each module exports its failure modes as `Error` — `vimage.Error`, `blas`/`lapack.Error`, `bnns.Error`, `sparse.Error`. vImage's raw `kvImage*` integers are `vimage.ErrorCode`, a separate namespace. |
| **Flags** | `vimage.Options`, a `packed struct(u32)`, never a bare integer. It cannot express a bit vImage does not define, and `kvImageUnknownFlagsBit` is a real failure. `Options.from(raw)` / `.bits()` cross an FFI boundary. |
| **Constructors** | `init`, or `init<Qualifier>` when a type has several. Free functions that build an object keep a verb name (`createCGImageFromBuffer`). |
| **Destructors** | `deinit(self: *T)`, always through a pointer, so `var x = try T.init(...); defer x.deinit();` is the shape everywhere. Where the handle is optional it is nulled out, which turns a double free into a no-op. |
| **Return values** | vImage wrappers return `Error!usize`. The `usize` is the scratch-buffer size when `.get_temp_buffer_size` is set and 0 otherwise — vImage reports a size and an error code through the same slot, so the success test is `>= 0`, not `== 0`. |
